...
class XGradCAM:
    ...
    def generate(self, input_tensor, class_idx=None, image_size=512, threshold=None):
        ...
        activations_pos = torch.relu(activations)

        weighted_grads = (activations_pos * grads).sum(dim=(1, 2)) 
        sum_activations = activations_pos.sum(dim=(1, 2))           

        weights = weighted_grads / (sum_activations + 1e-8)        
